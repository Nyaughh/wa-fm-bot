...
export default class extends BaseCommand {
    override execute = async (M: Message, {}: IParsedArgs): Promise<void> => {
        try {
            ...
            const formattedMemberList = memberList.map((member, index) => 
                `${index + 1}. ${member.username} (${member.lastfm}): ${member.playcount} scrobbles`
            );

            await M.reply(`Scrobbling Leaderboard in ${M.group?.title}:\n\n${formattedMemberList.join('\n')}\n\nTotal Scrobbles: ${totalScrobbles}`);
        } catch (error) {
