    ...
    override execute = async (M: Message, { text }: IParsedArgs): Promise<void> => {
        ...
        try {
            ...
            if (commonArtists.length > 0) {
                text += `\n\n*Common Artists:* ${commonArtists.slice(0, 5).join(', ')} ${
                    commonArtists.length > 5 ? `(${commonArtists.length - 5} more)` : ''
                }\n`
            }

            if (commonSongs.length > 0) {
                text += `\n*Common Songs:* ${commonSongs.slice(0, 5).join(', ')} ${
                    commonSongs.length > 5 ? `(${commonSongs.length - 5} more)` : ''
                }`
            }
