...
export default class extends BaseCommand {
    override execute = async (M: Message, { text }: IParsedArgs): Promise<void> => {
        ...
        const filteredData = data
            .filter((r): r is PromiseFulfilledResult<{ username: string, trackName: string, artistName: string, nowPlaying: boolean, jid: string, waname: string }> => r.status === 'fulfilled')
            .map((r) => r.value)
