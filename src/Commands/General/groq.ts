...
export default class extends BaseCommand {
    ...
    private async processAnthropicResponse(
        messages: typeof groupConversationHistory[string],
        M: Message
    ): Promise<string> {
                    ...
                    input_schema: {
                        ...
                        properties: {
                            ...
                            period: {
                                ...
                                description: 'The time period for the top songs'
                            }
                        },
                        required: ['username']
                    }
                    ...
                    input_schema: {
                        ...
                        properties: {
                            ...
                            period: {
                                type: 'string',
                                enum: ['overall', '7day', '1month', '3month', '6month', '12month'],
                                description: 'The time period for the top albums'
                            }
                        },
                        required: ['username']
                    }
    ...
    private async processToolCalls(toolCalls: ToolUseBlockParam[], M: Message): Promise<ToolResultBlockParam[]> {
        ...
        for (const toolCall of toolCalls) {
            ...
            if (toolCall.name === 'getlastfmRecs') {
                ...
                const { username, limit, period } = toolCall.input as {
                    username: string
                    limit: number
                    period: string
                }
                toolresponse = await this.getTopSongs(
                    username,
                    limit,
                ...
                toolresponse = await this.getTopAlbums(
                    username,
                    limit,
                    period as 'overall' | '7day' | '1month' | '3month' | '6month' | '12month'
                )
    ...
    private async getlastfmRecs(username: string, limit = 5) {
        ...
        const songs = response.data.playlist.slice(0, limit).map((song, index) => {
            return `${index + 1}. ${song._name} - ${song.artists
                .map((artist) => artist._name)
    ...
    private async getLastFmUsernameFromPhone(phone: string) {
        const user = await this.client.database.User.findOne({
            jid: phone.replace('@', '').replace('+', '').concat('@s.whatsapp.net')
        })
        if (!user?.lastfm) {
    ...
    private formatUserMessage(M: Message, userMessage: string, lastfmUsername: string): string {
        ...
        let quoted = ''
        if (M.quoted) {
            const text = M.quoted.message.extendedTextMessage
        ...
        return isGroup
            ? `${username}${lastfmUsername ? ` (Lastfm: ${lastfmUsername})` : ''}: ${userMessage} ${
